...
#         else:
        ...
        for i in range(len(loader['test'])):
            ...
            inputs = inputs.cuda()
            labels = labels.cuda()
            outputs = my_forward(model, inputs)
...
def my_forward(model, x):
    mo = nn.Sequential(*list(model.children())[9])
    feature = mo(x).view(-1, 512)
    transfer = model.transfer(feature)
    x = model.bottleneck(feature)
    x = model.bottleneck_new(x)
    x = x * transfer
...
#     else:
    ...
    for key, value in base_network.state_dict().items():
        print(key)
    # print(list(base_network.children())[9])
    # print(list(base_network.children())[10])
    # print(list(base_network.children())[11])
    # print(list(base_network.children())[9:12])

    #base_network = list(base_network.children())[9].cuda()
    # reshape = Reshape(512)
    # net_list = list(base_network.children())[:-1]
    # net_list.pop(9)
    # net_list.insert(9, reshape)
    # base_network= torch.nn.Sequential(*net_list).cuda()
