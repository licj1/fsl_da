...
import network_fc_attention as network
import loss
...
def train(config):
    ...
    new_dict = {k.replace('module.', ''): v for k, v in checkpoint['state_dict'].items() if
                (k.replace('module.', '') in model_dict.keys() and 'fc' not in k)}
    ...
    for i in range(config["num_iterations"]):
        ...
        pres = torch.cat((pre_source, pre_target), dim=0)
        ...
        if i % 10 == 0:
            print('iter: ', i, 'classifier_loss: ', classifier_loss.data, 'transfer_loss: ', transfer_loss.data)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='office',
                        choices=['office', 'image-clef', 'visda', 'office-home', 'imagenet'],
                        help="The dataset or source dataset used")
    parser.add_argument('--s_dset_path', type=str,
                        default='/mnt/lustre/dingmingyu/Research/da_zsl/dataset/imagenet/list/train_list.txt',
                        help="The source dataset path list")
    parser.add_argument('--t_dset_path', type=str,
                        default='/mnt/lustre/dingmingyu/Research/da_zsl/dataset/mini-imagenet/test_transfer_5.txt',
    ...
    if config["dataset"] == "office":
        ...
        config["optimizer"]["lr_param"]["lr"] = 0.001  # optimal parameters
        config["network"]["params"]["class_num"] = 448
        config['loss']["trade_off"] = 1.0
    else:
